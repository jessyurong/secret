...
function goToScene2() {
  state.name = nameInput.value.trim() || 'ikaw';
  greeting.innerHTML = TEXT_CONTENT.taglish.greeting(escapeHtml(state.name));
  showScene('scene2');
  resetNoButton();
}
...
function buildRecap() {
  ...
  const rows = [
    ...
    ['🚀 Ride', labelFor(TRANSPORTATION, state.transportation)],
...
// 15. SCENE 9 -> 10: Final Hugot (Sound plays synchronously right when Scene 10 appears!)
